...
export default class GanttChart extends LightningElement {
    ...
    dateShift = 7;
    // determines how many days we shift by
    ...
    setDateHeaders() {
        this.endDate = moment(this.startDate).add(this.view.slots * this.view.slotSize - 1, "days").toDate();
        ...
        getResources().then(resources => {
            ...
            this.resourceModalData = {
                ...
                resources: resources.filter(resource => {
                    ...
                    return {label: resource.Name, value: resource.Id, role: resource.Default_Role__c};
                })
            };
            window.console.log('Post Resources = ' + JSON.stringify(this.resourceModalData));
            this.template.querySelector(".resource-modal").show();
        }).catch(error => {
            this.dispatchEvent(new ShowToastEvent({message: error.body.message, variant: "error"}));
        });
    }
    ...
    filterProjects(event) {
        ...
        getProjects().then(projects => { // only show projects not selected
    ...
    filterProjectRecords(event) {
        ...
        getProjects().then(projects => { // only show projects not selected
            this.filterModalData.projerojectRecordTypeOptions = projects.filter(project => {
                return(project.RecordTypeId && !this.filterModalData.projects.filter(p => {
    ...
    addProjectRecordTypeFilter(event) {
        this.filterModalData.projectRecordTypes.push(Object.assign({}, event.currentTarget.dataset));
        this.filterModalData.focus = null;
    ...
    filterRoles(event) {
        ...
        this.filterModalData.roleOptions = this.roles.filter(role => {
            return(role.toLowerCase().includes(text.toLowerCase()) && !this.filterModalData.roles.filter(r => {
                return r === role;
            }).length);
        }).map(role => {
            return role;
    ...
    hideDropdowns() { // prevent menu from closing if focused
...
*/
    ...
    */
    ...
    handleRefresh() { // refreshApex(this.wiredData);
        let self = this;
